...
public class MemberRepoTests
{
    ...
    private Member MakeMember(Guid groupId, string name = "Alice") => new Member
    {
        Id = Guid.NewGuid(),
        Name = name,
    ...
    public async Task UpdateAsync_ChangeName_PersistsChange()
    {
        ...
        member.Name = "AliceUpdated";
        var result = await _repository.UpdateAsync(member);

        Assert.That(result.Name, Is.EqualTo("AliceUpdated"));
        Assert.That((await _context.Members.FindAsync(member.Id))!.Name, Is.EqualTo("AliceUpdated"));
    }
